...
import {
  ...
  ChevronLeft, ChevronRight, Target, Users, PlusCircle,
  TrendingUp
} from "lucide-react";
import { useNavigate } from "react-router-dom";
import { getUsers, deleteUser } from "../../services/getUser";
// ─── Paleta FocusMap ──────────────────────────────────────────────────────────
const FM = {
  orange:       "#ff9e3d",
  orangeDark:   "#f86f26",
...
export default function UsuariosList() {
  ...
  async function loadUsers() {
    try {
      ...
      if (!res.success) { setError(res.message || "Erro ao carregar usuários"); return; }
      setUsers(res.data ?? []);
    } catch {
    ...
    <div style={{ maxWidth: 960, margin: "0 auto", padding: "4px 0 3rem", display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {!loading && !error && filteredUsers.length === 0 && (
        <div style={{ textAlign: "center", padding: "64px 0", color: FM.textMuted }}>
          ...
          <p style={{ fontSize: 15, fontWeight: 600, color: FM.textMid, margin: "0 0 4px" }}>Nenhum usuário encontrado</p>
          <p style={{ fontSize: 13, margin: 0 }}>
            {searchTerm ? "Tente outro termo de busca" : "Adicione o primeiro usuário"}
          </p>
        </div>
      )}
      ...
      {!loading && currentUsers.length > 0 && (
        <div style={{ background: FM.bg, border: `1px solid ${FM.border}`, borderRadius: 12, overflow: "hidden" }}>
          ...
          <div style={{
            ...
            {["Usuário", "E-mail", "Ações"].map((col, i) => (
              <span key={col} style={{
...
<div style={{ display: "flex", justifyContent: "center", gap: 6 }}>
  <button
    onClick={() => navigate(`/dashboard/usuario/${user.id}/points`)}
    style={{
  ...
  {/* NOVO BOTÃO */}
  <button
    onClick={() => navigate(`/dashboard/user-progress/${user.id}`)}
    style={{
      display: "flex", alignItems: "center", gap: 5,
      padding: "5px 10px", fontSize: 12, fontWeight: 600,
